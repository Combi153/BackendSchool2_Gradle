...
class HospitalParserTest {
    ...
    void hospitalParsing() {
        ...
        assertHospital(hospital, "A1120837","서울특별시 금천구 벚꽃로 286 삼성리더스타워 111~114호 (가산동)","서울특별시 금천구","C",2,"가산기대찬의원", null);
    }

    @Test
    @DisplayName("insert쿼리 string을 잘 만드는지 test")
    void makeSqlQueryTest() {

        HospitalParser hospitalParser = new HospitalParser();
        Hospital hospital = hospitalParser.parse(this.line1);
        String sql ="('A1120837', '서울특별시 금천구 벚꽃로 286 삼성리더스타워 111~114호 (가산동)', '서울특별시 금천구', 'C', '2', '가산기대찬의원', 'null')";
        Assertions.assertEquals(hospital.toString(), sql);
    }
}